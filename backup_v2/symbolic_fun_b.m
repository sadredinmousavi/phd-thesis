function [] = symbolic_fun_b(values, fileName, vars)
...
for i=1:size(values,1)
    theta = atan(values(i,2)/values(i,1));
    m_hat = [
        sin( psai(i) ) * sin( vars.phi(i) )
        sin( psai(i) ) * cos( vars.phi(i) )
        cos( psai(i) )
        ];
    r_hat = [
        x - xx(i)
...
%     simplify(r_hat(1)^2+r_hat(2)^2+r_hat(3)^2);
    ...
    b_ = simplify( subs(B, [xx(i) yy(i) zz(i) m_norm mu_0], values(i,1:end-1)) );
    b_m_ = simplify( subs(m_norm_agent*B, [xx(i) yy(i) zz(i) m_norm mu_0 m_norm_agent], values(i,:)) );
    %
